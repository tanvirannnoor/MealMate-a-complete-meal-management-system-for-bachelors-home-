...
document.addEventListener("DOMContentLoaded", () => {
	...
	const { formatCurrency, formatDate, getUserById } =
	...
	document.getElementById(
		"reportWeek"
	).value = `${currentDate.getFullYear()}-W${weekNumber
		.toString()
		.padStart(2, "0")}`;
	...
	function getReportData(startDate, endDate) {
		...
		filteredExpenses.forEach((expense) => {
			expensesByMate[expense.payerId] =
				(expensesByMate[expense.payerId] || 0) +
	...
	function displayReport(reportData, reportType, startDate, endDate) {
		...
		document.getElementById("summaryTotalMeals").textContent =
			reportData.totalMeals;
		document.getElementById("summaryTotalExpenses").textContent =
			formatCurrency(reportData.totalExpenses);
		document.getElementById("summaryMealRate").textContent = formatCurrency(
			reportData.mealRate
		);
		...
		reportData.mateDetails.forEach((mate) => {
			...
			balanceCell.className = `px-4 py-3 ${
				mate.balance >= 0 ? "text-emerald-600" : "text-red-600"
			}`;
